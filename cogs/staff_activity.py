...
-------------------------
Requirement #7.
- Όταν μπαίνεις στο STAFF_ACTIVITY_VOICE_CHANNEL_ID -> παίρνεις ON_DUTY_ROLE_ID,
  ξεκινάει το χρονόμετρο.
- Όταν βγαίνεις -> σου αφαιρείται ο ρόλος, σταματάει το χρονόμετρο, προστίθεται
  στο σύνολο σου.
- Panel (Components V2) με leaderboard (top χρόνων) + λίστα ποιος είναι on-duty
  ΤΩΡΑ. Κουμπί Refresh για live ανανέωση.
- Logs με ώρα join/leave + διάρκεια session.
"""
...
STORE_NAME = "staff_activity"  # data/staff_activity.json -> {user_id: total_seconds}

# user_id -> datetime ξεκινήματος (in-memory, μηδενίζεται σε restart - αποδεκτό)
...
class StaffActivity(commands.Cog):
    ...
    def _build_panel_view(self, guild: discord.Guild) -> ui.LayoutView:
        store = storage.get_store(STORE_NAME)
        # συν τον τρέχον ενεργό χρόνο για όσους είναι on-duty αυτή τη στιγμή
        totals = dict(store)
        now = datetime.datetime.now(datetime.timezone.utc)
